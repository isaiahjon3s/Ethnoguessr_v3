...
def create_tables(path):
    ...
    c.execute("CREATE TABLE user_challenge (ID SERIAL PRIMARY KEY,FINISHED_ROUNDS INTEGER NOT NULL,SCORE INTEGER NOT NULL, USERID INTEGER REFERENCES users(id), CHALLENGEID INTEGER REFERENCES challenges(id), UNIQUE (CHALLENGEID, ROUND));")
